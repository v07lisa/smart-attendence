...
def TrackImages():
    ...
    i = 0
    j = 0
    recognizer = cv2.face.LBPHFaceRecognizer_create()  # ✅ Correct (if OpenCV contrib is installed)
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 100):
                ...
                I = df.loc[df['SERIAL NO.'] == serial]['ID'].values
                ID = str(I[0]) if len(I) > 0 else "Unknown"
                bb = str(aa)
                bb = bb[2:-2]
